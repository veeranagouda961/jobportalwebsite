...
export function ATSScorePanel() {
  ...
  const tier =
    score >= 71
      ? { color: "hsl(142, 50%, 40%)", label: "Strong Resume" }
      : score >= 41
      ? { color: "hsl(38, 70%, 50%)", label: "Getting There" }
      : { color: "hsl(0, 72%, 51%)", label: "Needs Work" };
        ...
        <div className="flex-1 space-y-1.5 max-h-[140px] overflow-y-auto">
          {suggestions.length === 0 ? (
            <div className="flex items-center gap-1.5 text-sm" style={{ color: tier.color }}>
              ...
              <span>Great job! Your resume covers all key areas.</span>
            </div>
          ) : (
            suggestions.map((s, i) => (
              <div key={i} className="flex items-start gap-1.5 text-xs text-muted-foreground">
                <AlertTriangle className="h-3 w-3 mt-0.5 shrink-0" style={{ color: tier.color }} />
                <span>{s}</span>
              </div>
            ))
          )}
